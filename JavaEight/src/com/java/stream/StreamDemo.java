...
Intermediate Operations – These are ones, which return another stream, and can be chained together.
For example filter(), sort(), map(), etc.

Terminal Operations – These are the ones, which produce a result from the pipeline. This result can be any non-stream value like List, Integer, void, etc.
forEach() and collect() methods are terminal operations.
